...
사진은 O(N^2) 와 O(NlogN)과 우선순위큐의 최악의 경우 O(N<sup>2</sup>logN)의 시간복잡도를 그래프로 나타낸 것이다.
...
이것을 빅오표기법으로 표시하면 각각 O(N<sup>2</sup>) 과 O(NlogN)이 나온다. 

또한 우선순위큐를 사용하였을 때, 일반적으로 O(V<sup>2</sup>)보다 O(ElogV)가 더 빠르지만 만일 최단거리를 찾고자 하는 그래프가 완전 그래프일 경우엔 E = V<sup>2</sup>이 되므로 일반적인 방식이 더 빨라 사용에 유의가 필요하다.

이것을 빅오 표기법으로 나타내 그래프에 그려보면 O(N<sup>2</sup>logN) 그래프가 그려진다.

하지만 평균적인 시간복잡도를 고려했을 때 우선순위 큐를 사용한 다익스트라 알고리즘의 시간복잡도가 일반적인 구현 방식의 시간복잡도 보다 더 적게 나온다는 것을 알 수 있다.
