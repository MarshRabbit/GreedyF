...
일반적인 배열을 사용한 방식은 모든 노드(N)에 대해 최소거리를 찾아야하기 때문에 노드의 제곱인 O(N<sup>2</sup>)가 걸린다.

여기서 우선순위 큐를 사용한다면 모든 간선에 대해서(O(E)) 큐를 업데이트(O(logN)) 하므로 O(ElogN)가 나온다.

일반적으로 O(N<sup>2</sup>)보다 O(ElogN)가 더 빠르지만 만일 최단거리를 찾고자 하는 그래프가 완전 그래프일 경우엔 E = N<sup>2</sup>이 되므로 일반적인 방식이 더 빨라 사용에 유의가 필요하다.
