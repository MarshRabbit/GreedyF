...
## 5. 문제점
이 기본적인 방식의 알고리즘을 사용하면 시간복잡도가 O(V<sup>2</sup>)이므로 노드의 개수가 많아지면 연산횟수가 커지는 단점이 존재한다.
하지만 우선순위 큐를 사용한다면 출발 노드와 가장 짧은 거리를 가진 노드를 먼저 꺼내게 되므로 시간복잡도를 획기적으로 줄일 수 있다.
